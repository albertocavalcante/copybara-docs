...
Test with folder destination using `--to-folder` and `--folder-dir`:
...
Or create a dedicated test workflow with `folder.origin()` and `folder.destination()` in your config.

## Next Steps

- [Text replacement](/transformations/replace/)
- [Filtering files](/transformations/filter/)
