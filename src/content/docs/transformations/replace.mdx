...
<Aside type="caution">
Use `[\\s\\S]*?` for non-greedy multiline matching. Plain `.*` doesn't match newlines.
</Aside>
...
<Aside type="note">
In Starlark strings, backslashes need double escaping: `\\.` becomes `\\.` in the regex.
</Aside>
...
- [Filtering & verification](/copybara-docs/transformations/filter/)
