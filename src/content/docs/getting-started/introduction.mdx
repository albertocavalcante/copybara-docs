...
source -> copybara -> dest
```

<Steps>
  1. **Read** code from the origin repository 2. **Transform** files (move,
  rename, replace content, filter) 3. **Write** to the destination repository
  (commit or PR) 4. **Track** state so only new changes are synced next time
</Steps>
...
<Aside type="tip">
  You can define both directions in the same config file with different workflow
  names.
</Aside>
...
- [Install Copybara](/getting-started/installation/)
- [Quick Start tutorial](/getting-started/quick-start/)
- [Understand core concepts](/getting-started/concepts/)
- [Glossary](/reference/glossary/) - Definitions of terms like Gerrit, CLs, Starlark, and more
