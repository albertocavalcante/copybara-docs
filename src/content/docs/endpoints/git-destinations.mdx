...
<Aside type="tip">
  Use `${CONTEXT_REFERENCE}` in `pr_branch` to create unique branch names based
...
<Aside type="caution" title="Experimental">
  GitLab MR destination support is experimental and the API may change.
</Aside>
...
| Parameter       | Required | Description                |
| --------------- | -------- | -------------------------- |
| `url`           | Yes      | GitLab repository URL      |
| `credentials`   | Yes      | Authentication credentials |
| `target_branch` | Yes      | Target branch for the MR   |
| `source_branch` | No       | Branch name for the MR     |
| `title`         | No       | MR title template          |
| `body`          | No       | MR description template    |
| `assignees`     | No       | GitLab usernames to assign |
...
This MR was created by Copybara.
...
<Aside type="note">
  Use `skip_push = True` during development to see what would be pushed without
