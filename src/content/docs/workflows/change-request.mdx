...
<Aside type="tip">
Use `update_description = True` to keep the PR description in sync with the latest changes.
</Aside>
...
<Steps>
1. **Use descriptive PR titles**: Include context about what's being synced
2. **Add labels**: Help reviewers identify automated PRs
3. **Exclude manual files**: Use `destination_files` to protect hand-edited content
4. **Verify no secrets**: Use `core.verify_match` to prevent leaks
5. **Test locally**: Use `folder.destination()` to preview changes
</Steps>
