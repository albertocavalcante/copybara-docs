...
<Aside type="note" title="PR vs MR vs CL">
  **PR** (Pull Request) is [GitHub](https://github.com/)'s term for a code
  review request. **MR** (Merge Request) is [GitLab](https://gitlab.com/)'s
  equivalent. **CL** (Change List) is
  [Gerrit](https://www.gerritcodereview.com/)'s term. Copybara supports all
  three systems. See the [Glossary](/reference/glossary/#cl-change-list) for
  more details.
</Aside>
...
destination: Destination commits {
  a_prime: A'
  b_prime: B'
  c_prime: C'
}
...
Creates a pull request (GitHub), merge request (GitLab), or change list (Gerrit) instead of pushing directly.
...
<TabItem label="Review-based">

Use **CHANGE_REQUEST** for review-based workflows (PRs, MRs, CLs):
...
<Aside type="note">
  You can also use `SQUASH` or `ITERATIVE` modes with PR/MR destinations - the
  mode controls how commits are created, while the destination controls where
  they go. `CHANGE_REQUEST` mode is specifically optimized for review workflows.
</Aside>
...
- [GitLab MR destination](/endpoints/gitlab/)
- [Gerrit CL destination](/endpoints/gerrit/)
