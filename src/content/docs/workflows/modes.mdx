...
<Aside type="note" title="PR vs CL">
  **PR** (Pull Request) is [GitHub](https://github.com/)'s term for a code
  review request. **CL** (Change List) is
  [Gerrit](https://www.gerritcodereview.com/)'s equivalent. Copybara supports
  both systems. See the [Glossary](/reference/glossary/#cl-change-list) for more
  details.
</Aside>
...
Use **CHANGE_REQUEST** for PR-based workflows:
...
<Aside type="note">
  You can also use `SQUASH` or `ITERATIVE` modes with PR destinations - the mode
  controls how commits are created, while the destination controls where they
  go. `CHANGE_REQUEST` mode is specifically optimized for PR workflows.
</Aside>
