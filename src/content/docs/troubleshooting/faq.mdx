...
**Not directly.** Copybara expects a single `copy.bara.sky` file.

From [Issue #268](https://github.com/google/copybara/issues/268):

> A user asked if there's a way to list replacement rules in multiple sky files and invoke those files from the main copy.bara.sky file.

**Workaround:** Define reusable functions within the same file:
